...
public class BungeeMethods implements MethodInterface {
    ...
    public boolean isOnline(String name) {
        try {
            if (Universal.isRedis()) {
                for (String str : RedisBungeeAPI.getRedisBungeeApi().getHumanPlayersOnline()) {
                    if (str.equalsIgnoreCase(name)) {
                        return RedisBungeeAPI.getRedisBungeeApi().getPlayerIp(RedisBungeeAPI.getRedisBungeeApi().getUuidFromName(str)) != null;
                    }
                }
            }
    ...
    public void kickPlayer(String player, String reason) {
        if(BungeeMain.getCloudSupport() != null){
            ...
            RedisBungeeAPI.getRedisBungeeApi().sendChannelMessage("advancedban:main", "kick " + player + " " + reason);
        } else {
    ...
    public void notify(String perm, List<String> notification) {
        if (Universal.isRedis()) {
            notification.forEach((str) -> RedisBungeeAPI.getRedisBungeeApi().sendChannelMessage("advancedban:main", "notification " + perm + " " + str));
        } else {
