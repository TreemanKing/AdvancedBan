...
import me.leoko.advancedban.bungee.FakeServer;
import me.leoko.advancedban.bungee.FakeServerInfo;
...
public class ChatListenerBungee implements Listener {
    ...
    public void onChat(ChatEvent event) {
        ...
        if (event.getReceiver() instanceof FakeServer) {
            System.out.println(event.isCancelled());
            return;
        }
        if (!event.isCommand()) {
            if (Universal.get().getMethods().callChat(event.getSender(), event.getMessage())) {
                event.setCancelled(true);

                if (!Universal.get().getMethods().isSoftMuted(event.getSender())) return;
